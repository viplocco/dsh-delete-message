...
> 覆盖范围：v0.1.0 开发完成 → 多轮回归修复与发版（v0.1.1→v0.2.3）→ npm 发布 → dsh-web 社区插件索引登记 PR #1185。
...
| 宿主适配 | v0.2.3 | 用户/助手按钮双失效回归修复（桌面宿主 UI 结构迁移） |
...
### 5. v0.2.3：宿主 UI 结构迁移 → 用户/助手删除按钮双失效

- **症状**：用户消息复制按钮右侧始终没有删除图标；助手消息删除按钮始终灰（`opacity .4`，点击完全无效）。
- **根因（两条独立宿主结构错配，全在 client）**：① DOM 增强入口依赖已废弃的 `data-time-hover-root`，当前宿主（`dsh-client-ui-chat@0.1.2-rc.1`）改用 `data-chat-flow-kind` 标识用户行，且操作条从"直接子"变成 `wrapper > div.userRow > div.*_actions` 的**孙子级**——入口不命中 + 下钻只查直接子，垃圾桶永不挂载；② `findSeqByMessageId` 只查 `data.finalNode`/steering，不解析 turn-tail 的 `data.closing.finalNode`，压缩回合解析失败 → `disabled` 置灰。
- **教训**：DOM 增强这类"搭在宿主渲染结构上的桥"，必须探测**宿主的现行标记**（本插件假设的 `data-time-hover-root` 是更早宿主版的产物，宿主升级 UI 结构后静默失效，无报错无日志只有图标不出现）。迁移到宿主自用标记 `data-chat-flow-kind` 后即随宿主演进自洽；升级路径（宿主给 user 侧开对称槽位后整体退役 DOM 增强）不变。
